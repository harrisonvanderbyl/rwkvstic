...
class Block(torch.nn.Module):
            def __init__(self, dims,T):
                ...
                self.attkey.weight =torch.nn.Parameter(torch.rand_like(self.attkey.weight))
                self.attvalue.weight =torch.nn.Parameter(torch.rand_like(self.attvalue.weight))
                self.attreceptance.weight =torch.nn.Parameter(torch.rand_like(self.attreceptance.weight))
                self.ffnkey.weight =torch.nn.Parameter( torch.rand_like(self.ffnkey.weight))
                self.ffnvalue.weight =torch.nn.Parameter(torch.rand_like(self.ffnvalue.weight))
                self.attout.weight =torch.nn.Parameter(torch.rand_like(self.attout.weight))
                self.ffnreceptance.weight =torch.nn.Parameter(torch.rand_like(self.ffnreceptance.weight))
                self.attmixk  =torch.nn.Parameter( torch.rand_like(self.attmixk))
                self.attmixv  =torch.nn.Parameter( torch.rand_like(self.attmixv))
                self.attmixr  =torch.nn.Parameter(   torch.rand_like(self.attmixr))
                self.ffnmixk  =torch.nn.Parameter(torch.rand_like(self.ffnmixk))
                self.ffnmixr  =torch.nn.Parameter(torch.rand_like(self.ffnmixr))
                self.wkv.time_first =torch.nn.Parameter(torch.rand_like(self.wkv.time_first))
                self.wkv.time_decay =torch.nn.Parameter(torch.rand_like(self.wkv.time_decay))

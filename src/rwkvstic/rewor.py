...
class Block(torch.nn.Module):
    ...
    def processLayer(self, ki, v, rz: List[torch.Tensor], state, i: int):
    ...
    def forward(self, x, state):
        ...
        for i in range(len(k)):
            rz, state = self.processLayerx(k[i], v[i], rz, state, i)
...
out = myrwkv.forward(torch.LongTensor([2]), torch.zeros(12, 5, 768))
error = out.sum()
torch.autograd.set_detect_anomaly(True)
error.backward()

# myrwkv.loadFromBlinkDLCheckpoint(
#     "/home/harrison/projects/rwkv-chat/RWKV-4-Pile-169M-20220807-8023.pth"
# )

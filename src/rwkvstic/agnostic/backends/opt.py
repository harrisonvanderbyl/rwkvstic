...
def OptRWKV(path, jit=True  , export=False, **kwargs):
    ...
    class myRWKV(RwkvModule):
        ...
        def __init__(self,w,dims,layers):
            ...
            vustatea = torch.stack([self.emptyState[i][0] for i in range(layers)]).double().contiguous().cuda()
            vustateb = torch.stack([self.emptyState[i][1] for i in range(layers)]).double().contiguous().cuda()
            vustatec = torch.stack([self.emptyState[i][2] for i in range(layers)]).double().contiguous().cuda()
            vustated = torch.stack([self.emptyState[i][3] for i in range(layers)]).double().contiguous().cuda()
            vustatee = torch.stack([self.emptyState[i][4] for i in range(layers)]).double().contiguous().cuda()
            self.emptyState = torch.stack([vustatea,vustateb,vustatec,vustated,vustatee])
            ...
            for key in toQuantize:
                ...
                keysp = key.split(".")[:2]
                keysp = "".join(keysp)                
                self.__dict__[keysp+"weights"] = torch.stack([x[0] for x in weights], dim=0).to(dtype=torch.uint8, memory_format=torch.contiguous_format, device="cuda")
                self.__dict__[keysp+"ranges"] = torch.stack([x[1] for x in weights], dim=0).to(dtype=torch.float32, memory_format=torch.contiguous_format, device="cuda")
                self.__dict__[keysp+"zp"] = torch.stack([x[2] for x in weights], dim=0).to(dtype=torch.float32, memory_format=torch.contiguous_format, device="cuda")
                import gc
        ...
        def forward(self, x, state):
            ...
            torch.ops.rwkv.rwkvc(
                ...
                0,
                self.rx,
                x.clone().double().cuda().contiguous(),
                self.cudalnin,
                state[0],
                state[1],
                state[2],

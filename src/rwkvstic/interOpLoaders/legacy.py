...
class RWKV_RNN(MyModule):
    def __init__(self, model_name, float_mode, run_device):
        super().__init__()
        ...
        with torch.no_grad():
            ...
            if 'pos_emb_x' in keys:
                w['pos_emb'] = (w['pos_emb_x'] + w['pos_emb_y']
                                ).reshape(self.args.ctx_len+1, -1)[:-1, :]
            keys = list(w.keys())
        ...
        self.args.n_embd = self.w.blocks[0].ffn.time_mix_k.shape[0]
        self.args.n_layer = len(self.w.blocks)
